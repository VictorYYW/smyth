...
let command_description : command -> string =
  ...
  | SolveExs ->
    "Complete a program sketch but take assertions from a separate file"
...
let command_arguments : command -> (string * string) list =
  ...
  | SolveExs ->
    [ 
      ...
      ( "examples"
      , "examples in string format to be parsed"
      )
    ]
  ...
  if Compilation2.is_js then
    ...
    begin
      ...
      begin match command with
        ...
        | SolveExs ->
          begin
            let builtin = Sys.argv.(2) in
            let rec concat_L_input : Lang.exp -> Lang.exp = function
              | ECtor ("Nil", [], ETuple []) -> ECtor ("LNil", [], ETuple [])
              | ECtor ("Cons", [], ETuple [hd; tl]) -> ECtor ("LCons", [], ETuple [hd; concat_L_input tl])
              | _ -> failwith "impossible"
            in
            let exs = Parser_wrapper.parse_examples (Sys.argv.(3))
                      |> List.map (fun (i, o) ->
                          (match i with
